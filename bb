...
function OrionLib:MakeWindow(WindowConfig)
    ...
    function SelectSubTab()
        ...
        for _, child in pairs(SubTabContainer:GetChildren()) do
            if not (child:IsA("UIListLayout") or child:IsA("UIPadding")) then
                -- Only make non-section elements visible
        ...
        local buttonDownEffect = function()
        ...
        local buttonUpEffect = function()
            TweenService:Create(SubTabButton, TweenInfo.new(0.08, Enum.EasingStyle.Quart), {
                Size = UDim2.new(0, SubTabButton.Size.X.Offset, 0, 30)
            }):Play()
        end
        
        AddConnection(SubTabButton.MouseButton1Down, buttonDownEffect)
        AddConnection(SubTabButton.MouseButton1Up, buttonUpEffect)
        AddConnection(SubTabButton.MouseButton1Click, SelectSubTab)
